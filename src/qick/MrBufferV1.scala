...
case class MrBufferIO(
    ...
    N: Int = 8,
    B: Int = 16,
    C_S00_AXI_DATA_WIDTH: Int = 32,
    C_S00_AXI_ADDR_WIDTH: Int = 6,
    DEBUG: Int = 0
) extends Bundle {
  val trigger = in Bool ()
  val s00_axi_lite = slave port AxiLite4(
    AxiLite4Config(
      dataWidth = C_S00_AXI_DATA_WIDTH,
      addressWidth = C_S00_AXI_ADDR_WIDTH
    )
  )
  ...
  val s00_axis = slave port Axi4Stream(
    Axi4StreamConfig(dataWidth = NM * B / 8, useStrb = true, useLast = true)
  )
  ...
  s00_axis.data.addAttribute("X_INTERFACE_PARAMETER", "FREQ_HZ 307200000")
  s00_axis.strb.setName("s00_axis_tstrb")
  s00_axis.last.setName("s00_axis_tlast")
  s00_axis.valid.setName("s00_axis_tvalid")
  val m00_axis = master port Axi4Stream(
    Axi4StreamConfig(dataWidth = B / 8, useStrb = true, useLast = true)
  )
  ...
  m00_axis.data.addAttribute("X_INTERFACE_PARAMETER", "FREQ_HZ 99999985")
  m00_axis.strb.setName("m00_axis_tstrb")
  m00_axis.last.setName("m00_axis_tlast")
  m00_axis.valid.setName("m00_axis_tvalid")
  val s_dbg_probe = (DEBUG == 1) generate (out Bool ())
  val m_dbg_probe = (DEBUG == 1) generate (out Bool ())
}
...
case class mr_buffer_v1_0(
    ...
    N: Int,
    B: Int,
    C_S00_AXI_DATA_WIDTH: Int,
    C_S00_AXI_ADDR_WIDTH: Int,
    DEBUG: Int
) extends BlackBox {
  // addGeneric("NM", NM)
  // addGeneric("N", N)
  ...
  val io =
    MrBufferIO(NM, N, B, C_S00_AXI_DATA_WIDTH, C_S00_AXI_ADDR_WIDTH, DEBUG)
  noIoPrefix()
}
...
case class MrBufferV1(
  ...
  val io =
    MrBufferIO(NM, N, B, C_S00_AXI_DATA_WIDTH, C_S00_AXI_ADDR_WIDTH, DEBUG)
  val mrbuffer =
    mr_buffer_v1_0(NM, N, B, C_S00_AXI_DATA_WIDTH, C_S00_AXI_ADDR_WIDTH, DEBUG)
  noIoPrefix()
  io <> mrbuffer.io
}
